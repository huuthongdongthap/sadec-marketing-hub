#!/usr/bin/env python3
"""
SEO Metadata Adder for Sa Đéc Marketing Hub
Tự động thêm SEO metadata (Open Graph, Twitter Card, JSON-LD) cho tất cả HTML files
"""
...
SEO_METADATA = {
    ...
    "admin/content-calendar.html": {
        ...
        "description": "Lên lịch và quản lý nội dung đăng tải tự động.",
        "type": "website"
    },
...
def process_html_file(file_path: str) -> Tuple[bool, str]:
    """Xử lý một file HTML, thêm SEO metadata"""
    try:
        ...
        # Kiểm tra xem đã có SEO metadata chưa
        has_og_title = 'property="og:title"' in content
        has_title_tag = bool(re.search(r'<title[^>]*>.*?</title>', content, re.IGNORECASE | re.DOTALL))

        if has_og_title:
            return False, "Đã có OG metadata"

        # Tạo SEO block
        seo_block = generate_seo_block(file_path, metadata)

        # Tìm vị trí chèn (sau </head> hoặc sau các meta tags cơ bản)
        # Ưu tiên: chèn trước khi đóng </head>
        head_close = re.search(r'</head>', content, re.IGNORECASE)
        if head_close:
            # Chèn tất cả metadata trước </head>
            new_content = content[:head_close.start()] + seo_block + "\n" + content[head_close.start():]
        else:
            # Không tìm thấy </head>, trả về lỗi
            return False, "Không tìm thấy </head> tag"

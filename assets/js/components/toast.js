 ...
 * Lightweight toast notifications với animations
...
class Toast extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['type', 'message', 'duration', 'dismissible'];
  }
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        :host([type="error"]) {
          background: var(--md-sys-color-error-container, #900020);
        }

        :host([type="warning"]) {
          background: var(--md-sys-color-tertiary-container, #5B14AC);
        }

        :host([type="info"]) {
          background: var(--md-sys-color-secondary-container, #006E1C);
        }
      ...
      <span class="icon material-symbols-outlined">${icons[type] || icons.info}</span>
      <span class="content">${message}</span>
      ${dismissible ? `<button class="dismiss-btn" aria-label="Đóng">
        <span class="material-symbols-outlined">close</span>
      </button>` : ''}
      ${this.getAttribute('duration') ? `<div class="progress" style="animation-duration: ${this.getAttribute('duration')}ms"></div>` : ''}
    `;
  }
  ...
  setupAutoHide() {
    this.clearHideTimeout();
    const duration = this.getAttribute('duration') || this.autoHideDelay;
    
    if (duration && duration !== '0') {
      this.hideTimeout = setTimeout(() => this.dismiss(), parseInt(duration, 10));
    }
  }
  ...
  dismiss() {
    const toast = this.shadowRoot.host;
  ...
  static show(options = {}) {
    ...
    // Auto remove from container when dismissed

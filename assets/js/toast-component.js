 ...
 * ═══════════════════════════════════════════════════════════════════════════
 * Standalone toast notification web component
 *
 * Usage:
 *   <toast-component type="success" title="Thành công" message="Đã lưu">
 *   </toast-component>
 *
 *   Or via JS:
 *   ToastComponent.show('Success', 'Message', 'success');
 */
...
class ToastComponent extends HTMLElement {
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="toast" role="alert" aria-live="polite">
        ...
        <button class="toast-dismiss" aria-label="Đóng" type="button">
   ...
   */
  addEventListeners() {
    const dismissBtn = this.shadowRoot.querySelector('.toast-dismiss');
    if (dismissBtn) {
    ...
    if (toast) {
      ...
      toast.addEventListener('touchend', () => {
        if (Math.abs(touchCurrentX - touchStartX) > 100) {
          this.dismiss();
        } else {
  ...
  getStyles() {
    return `
      ...
      .toast {
        ...
        border-left: 4px solid var(--toast-color, #1976d2);
      ...
      :host([type="success"]) .toast {
        --toast-color: var(--md-sys-color-tertiary, #4caf50);
      }

      :host([type="error"]) .toast {
        --toast-color: var(--md-sys-color-error, #f44336);
      }

      :host([type="warning"]) .toast {
        --toast-color: var(--md-sys-color-error-container, #ff9800);
      }

      :host([type="info"]) .toast {
        --toast-color: var(--md-sys-color-primary, #1976d2);
      }
      ...
      @media (prefers-color-scheme: dark) {
        .toast {
          background: var(--md-sys-color-surface-dark, #1e1e1e);
        }

        .toast-title {
          color: var(--md-sys-color-on-surface-dark, #fff);
        }

        .toast-message {
          color: var(--md-sys-color-on-surface-variant-dark, #aaa);
        }
      }
    `;
  }

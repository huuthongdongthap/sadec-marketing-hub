...
export async function loadDashboard() {
    try {
        // Check for BinhPhapAPI (from supabase-config.js)
        ...
        // Fallback: Use AdminAPI if available
        if (window.AdminAPI) {
            const stats = await loadDashboardFromAPI();
            updateDashboardStats(stats);
        }
...
/**
 * Load dashboard stats from AdminAPI
 */
async function loadDashboardFromAPI() {
    const [customers, projects, invoices, campaigns] = await Promise.all([
        window.AdminAPI.loadCustomers({ limit: 0 }),
        window.AdminAPI.loadPosts ? window.AdminAPI.loadPosts({ limit: 0 }) : { data: [] },
        window.AdminAPI.loadCustomers ? window.AdminAPI.loadCustomers({ status: 'active' }) : { data: [] },
        { data: [] }
    ]);
    ...
    return {
        ...
        pending_revenue: invoices.data?.reduce((sum, c) => sum + (c.value || 0), 0) || 0,
...
export function updateDashboardStats(stats) {
    ...
    if (elCustomers) {
        animateCountUp(elCustomers, stats.total_customers || 0);
    }

    // Active projects
    const elProjects = document.getElementById('stat-projects');
    if (elProjects) {
        animateCountUp(elProjects, stats.active_projects || 0);
    }
    ...
    // Active campaigns
    const elCampaigns = document.getElementById('stat-campaigns');
    if (elCampaigns) {
        animateCountUp(elCampaigns, stats.active_campaigns || 0);
    }
}
 ...
 */
export function animateCountUp(element, targetValue, duration = 1500, suffix = '') {
    if (!element) return;
...
/**
 * Create sparkline chart SVG
 */
export function createSparkline(data, trend = 'up') {

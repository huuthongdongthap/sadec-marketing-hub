/**
 * ═══════════════════════════════════════════════════════════════════════════
 * PREMIUM INTERACTIONS — Sa Đéc Marketing Hub
 *
 * JavaScript-powered premium animations and interactions
 *
 * Usage:
 *   <script type="module" src="/assets/js/premium-interactions.js"></script>
 *
 * @version 1.0.0 | 2026-03-14
 * @module premium-interactions
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
class RippleEffect {
    constructor(selector = '.premium-btn-ripple') {
    ...
    static init(selector) {
        return new RippleEffect(selector);
    }
}
...
class TiltEffect {
    constructor(selector = '.premium-card-tilt') {
        this.selector = selector;
        this.elements = [];
        this.init();
    }
    ...
    static init(selector) {
        return new TiltEffect(selector);
    }
}
...
class ParallaxScroll {
    constructor(selector = '.premium-parallax') {
        this.selector = selector;
        this.elements = [];
        this.init();
    }
    ...
    onScroll() {
        ...
        this.elements.forEach((item) => {
            ...
            const offset = (scrolled - rect.top) * item.speed;
            item.element.style.transform = `translateY(${offset}px)`;
        });
    }

    static init(selector) {
        return new ParallaxScroll(selector);
    }
}
...
class MagneticButton {
    constructor(selector = '.premium-magnetic') {
        this.selector = selector;
        this.init();
    }
    ...
    init() {
        ...
        buttons.forEach((button) => this.bindEvents(button));
    }
    ...
    static init(selector) {
        return new MagneticButton(selector);
    }
}
...
class TextReveal {
    constructor(selector = '.premium-text-reveal') {
        this.selector = selector;
        this.init();
    }
    ...
    init() {
        const elements = document.querySelectorAll(this.selector);
        elements.forEach((el) => this.splitText(el));
    }
    ...
    splitText(element) {
        ...
        text.split('').forEach((char, index) => {
            ...
            span.style.cssText = `
                ...
                animation: textRevealAnim 0.5s ease forwards;
    ...
    static init(selector) {
        return new TextReveal(selector);
    }
}
...
class ScrollProgress {
    constructor(selector = '.premium-scroll-progress') {
        this.selector = selector;
    ...
    init() {
        this.progressBar = document.querySelector(this.selector);
    ...
    static init(selector) {
        return new ScrollProgress(selector);
    }
}
...
class LazyLoadAnimate {
    constructor(selector = '.premium-lazy') {
        this.selector = selector;
        this.init();
    }
    ...
    static init(selector) {
        return new LazyLoadAnimate(selector);
    }
}
...
export { PremiumInteractions };
export default PremiumInteractions;

// Auto-init
PremiumInteractions.initAll();
